...
class GuangdongScraper:
    def __init__(self):
        """初始化广东爬虫"""
        self.base_url = "https://amr.gd.gov.cn/zwgk/zdlyxxgk/ajgs/laxx/index.html"
    ...
    async def parse_list_page(self, url):
        ...
        try:
            ...
            # 获取案件链接
            cases = []
            case_items = soup.select('ul.news_list2.marB20 li')
            ...
            if 'index.html' in current_url_parts[-1]:
                current_page = 1
                next_page = url.replace('index.html', 'index_2.html')
            else:
        ...
        try:
            ...
            if date_match:
                try:
                    ...
                    if '日' not in start_date_str:
                        start_date_str += '日'
                    start_date = datetime.strptime(start_date_str, '%Y年%m月%d日')
                    
                    # 处理结束日期
                    end_date = datetime.strptime(date_match.group(2) + '日', '%Y年%m月%d日')
            ...
            attachment_link = content_div.select_one('a.nfw-cms-attachment')
            if attachment_link:
                attachment_url = attachment_link.get('href')
                attachment_name = attachment_link.get('download')
    ...
    async def process_case(self, case):
        ...
        try:
            ...
            if existing_case:
                ...
                # 检查并更新地区
                if existing_case.region != '广东':
                    logger.info(f"更新案件地区 [{case_title}]: {existing_case.region} -> 广东")
                    existing_case.region = '广东'
                    need_update = True
                ...
                try:
                    ...
                    for case_name in current_df.index:
                        if case_name in existing_df.index:
                            ...
                            if existing_case['地区'] != '广东':
                                existing_df.at[case_name, '地区'] = '广东'
...
if __name__ == "__main__":
    ...
    scraper = GuangdongScraper()
    # 运行爬虫，设置最高页码为 8
    logger.info("开始运行广东爬虫，限制页码为 8 页...")
    asyncio.run(scraper.run(max_page=8))
    logger.info("爬虫运行结束（限制 8 页）。")
